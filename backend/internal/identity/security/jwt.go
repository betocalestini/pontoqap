...
func IssueStoreToken(secret string, userID, sessionID uuid.UUID, expiresAt time.Time) (string, error) {
	if secret == "" {
		return "", fmt.Errorf("jwt secret required")
	}
	now := time.Now()
	claims := StoreClaims{
		RegisteredClaims: jwt.RegisteredClaims{
			Subject:   userID.String(),
			Audience:  jwt.ClaimStrings{StoreJWTAudience},
			ExpiresAt: jwt.NewNumericDate(expiresAt),
			IssuedAt:  jwt.NewNumericDate(now),
		},
		SID: sessionID.String(),
...
func ParseStoreToken(secret, raw string) (userID, sessionID uuid.UUID, err error) {
	if secret == "" {
		return uuid.Nil, uuid.Nil, fmt.Errorf("jwt secret required")
	}
	parsed, err := jwt.ParseWithClaims(raw, &StoreClaims{}, func(t *jwt.Token) (any, error) {
		if t.Method != jwt.SigningMethodHS256 {
			return nil, fmt.Errorf("unexpected signing method")
		}
		return []byte(secret), nil
	}, jwt.WithAudience(StoreJWTAudience))
	if err != nil {
		return uuid.Nil, uuid.Nil, err
	}
	claims, ok := parsed.Claims.(*StoreClaims)
	if !ok || !parsed.Valid {
		return uuid.Nil, uuid.Nil, fmt.Errorf("invalid token")
	}
	userID, err = uuid.Parse(claims.Subject)
