...
func TestCollaboratorCheckoutLowerPrice(t *testing.T) {
	...
	salesSvc := sales.NewService(pool, inv, billing.NewService(pool, nil, "", nil), cat, custSvc, nil)
...
func TestBlockedCustomerCheckout(t *testing.T) {
	...
	salesSvc := sales.NewService(pool, inv, billing.NewService(pool, nil, "", nil), catalog.NewService(pool), custSvc, nil)
